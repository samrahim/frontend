...
import {
  useUpdateRecitationMutation,
  useRecitationsQuery,
  RecitationsDocument,
} from "../graphql";

interface RecitationForm {
  studentID: string;
...
export function EditRecitationPage() {
  ...
  // Form State
  const [formData, setFormData] = useState<RecitationForm>({
    studentID: "",
    studentfirstName: "",
    studentlastName: "",
    teacherID: "",
  ...
  const [updateRecitation] = useUpdateRecitationMutation({
    refetchQueries: [
      {
        query: RecitationsDocument,
        variables: { first: 10, after: null, where: undefined },
      },
    ],
  });

  const recitation = recitationDetails?.recitations?.edges?.[0]?.node;

  // Track initial server values to dynamically evaluate if changes exist
  const initialValues = useMemo(() => {
    if (!recitation) return null;

    const foundSurah = surahs.find((s) => s.name_ar === recitation.surah);

    return {
      studentID: recitation.student?.id || "",
      studentfirstName: recitation.student?.firstName || "",
      studentlastName: recitation.student?.lastName || "",
      teacherID: recitation.teacher?.id || "",
      teacherfirstName: recitation.teacher?.firstName || "",
      teacherlastName: recitation.teacher?.lastName || "",
      surahId: foundSurah ? foundSurah.number : 0,
      surah: recitation.surah || "",
      fromAyah: recitation.fromAyah != null ? String(recitation.fromAyah) : "",
      toAyah: recitation.toAyah != null ? String(recitation.toAyah) : "",
      evaluation: recitation.evaluation?.toString() || "",
      notes: recitation.note || "",
      type: recitation.type || "NEW_RECITATION",
  ...
  // Submit Logic matching UpdateRecitationInput specifications
  ...
  if (error)
    return (
      <Layout>
        <Box sx={{ p: 4, color: "red" }}>Error loading recitation details</Box>
      </Layout>
    );
